...
random_search = RandomizedSearchCV(
    ...
    scoring='accuracy',
    n_jobs=1,  # Set to 1 to run serially
    verbose=2,
    random_state=42
)
...
feature_names = (
    ...
    .transformers_[0][1]
    .named_steps['scaler']
    .fit(X[numerical_features])
    .transform(X[numerical_features]).columns.tolist() + 
    list(random_search.best_estimator_.named_steps['preprocessor']
         .transformers_[1][1]
         .named_steps['onehot']
...
# Optional: Look for common misclassifications
misclassification_counts = misclassified.groupby(['Actual', 'Predicted']).size().reset_index(name='Counts')
