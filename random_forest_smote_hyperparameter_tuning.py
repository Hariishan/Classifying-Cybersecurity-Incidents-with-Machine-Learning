...
numerical_features = X_resampled.select_dtypes(include=['float32', 'int32']).columns
categorical_features = X_resampled.select_dtypes(include=['category']).columns
...
start_time = time.time()
random_search.fit(X_resampled, y_resampled)  # Use resampled dataset for RandomizedSearchCV
end_time = time.time()
...
start_time = time.time()
y_pred = random_search.best_estimator_.predict(X_resampled)
end_time = time.time()
...
cv_scores = cross_val_score(random_search.best_estimator_, X_resampled, y_resampled, cv=5, scoring='accuracy')
